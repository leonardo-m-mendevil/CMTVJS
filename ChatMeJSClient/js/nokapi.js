...
knocknok.Tag.prototype.launch = function () {
    ...
    var opts = StringTools.urlEncode(knocknok.Base64.encode(hxJSON.Json.encode({
        topicId: topic.id,
...
events = {}
events.EventDispatcher = function (p) {
    if (p === $_) return; {
...
knocknok.WebRequestBase = function (url, method) {
    if (url === $_) return; {
...
knocknok.WebRequest = function (url, method) {
    if (url === $_) return; {
        if (method == null) method = "GET";
        method = "GET";
...
knocknok.ApiRequest = function (path, method) {
    if (path === $_) return; {
        if (method == null) method = "GET";
...
knocknok.Session = function (sourceId) {
    if (sourceId === $_) return; {
        events.EventDispatcher.apply(this, []);
        this.endTime = null;
        this.screenName = "";
...
events.TopicEvent = function (type, topic) {
    if (type === $_) return; {
...
knocknok.TermInfo = function (text, classification, relevancy) {
    if (text === $_) return; {
        this.text = text;
...
events.SourceEvent = function (type, source) {
    if (type === $_) return; {
        events.Event.apply(this, [type]);
        this.source = source;
    }
...
hxJSON._Json = {}
hxJSON._Json.Encode = function (value) {
    if (value === $_) return; {
...
hxJSON._Json.Encode.prototype.convertToString = function (value) {
    if (Std["is"](value, String)) {
        ...
        haxe.Log.trace("process a list", {
            fileName: "Json.hx",
            lineNumber: 70,
            className: "hxJSON._Json.Encode",
...
hxJSON._Json.Decode = function (t) {
    if (t === $_) return; {
...
events.WebRequestEvent = function (type, data) {
    if (type === $_) return; {
        events.Event.apply(this, [type]);
        this.data = data;
    }
...
knocknok.Message = function (txt, topic, subscriber) {
    if (txt === $_) return; {
...
knocknok.ConnectionBase = function (topic, sess) {
    if (topic === $_) return; {
        events.EventDispatcher.apply(this, []);
        this.topic = topic;
        this.session = sess;
...
knocknok.Connection = function (topic, sess) {
    if (topic === $_) return; {
...
events.MessageEvent = function (type, message) {
    if (type === $_) return; {
        events.Event.apply(this, [type]);
...
knocknok.User = function (session, sn, source) {
    if (session === $_) return; {
        this.source = source;
...
events.UserEvent = function (type, user) {
    if (type === $_) return; {
        events.Event.apply(this, [type]);
        this.user = user;
    }
...
events.SessionEvent = function (type) {
    if (type === $_) return; {
        events.Event.apply(this, [type]);
    }
}
...
events.ConnectionEvent = function (type) {
    if (type === $_) return; {
        events.Event.apply(this, [type]);
    }
}
...
knocknok.PollRequest = function (conn) {
    if (conn === $_) return; {
...
knocknok.Source = function (id, host, title, url, terms, referrals) {
    if (id === $_) return; {
